...
`.build()` runs the array validator before constructing the `Namefully` unless `Config.mono` is on. So a builder with no `LastName` will throw at build time — and that's intentional, since silently producing a `Namefully` with missing required parts is worse than telling you to add it.

If you want a builder for mononyms, pass `{ mono: true }` to `.build()`:

```ts
NameBuilder.of(Name.first('Cher')).build({ mono: true });
```
