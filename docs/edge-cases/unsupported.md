...
## Mononyms (without the flag)

```ts
new Namefully('Plato'); // throws
```

A one-token name needs `Config.mono` turned on. See the [`mono` option](../configuration.md#mono) and the [Mononyms section in the migration guide](../migrating-from-v1.md#mononyms).
